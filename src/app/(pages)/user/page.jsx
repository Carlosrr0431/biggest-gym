...
const User = () => {
  ...
  useEffect(() => {
    ...
    if (session?.user?.email != undefined) getSupabaseOficial();
  ...
  useEffect(() => {
    if (usuario?.tipoPlan == "Plan x2") {
      ...
      divRef.current.style.width = `${diasPlan}%`;
    } else divRef.current.style.width = `0%`;
          ...
          <div className="p-5 py-2 text-left transform duration-500 hover:-translate-y-1 hover:shadow-xl cursor-pointer bg-black/50">
            <div className="flex justify-start gap-x-4">
              <Image
                ...
                height={0}
              />
              <h2 className="font-semibold mb-2 mt-4 w-full text-white/70 text-[20px]">
                Dias cumplidos
              </h2>
            </div>

            <h2 className="font-semibold mb-2 mx-auto text-center  w-full text-yellow-500 text-[30px]">
              {usuario?.dias}
            </h2>
            {usuario?.dias >= cantDias && usuario?.tipoPlan !== "" ? (
              <h2 className="font-semibold mb-2 mx-auto text-center  w-full text-red-400 text-[15px]">
                Necesitas renovar tu plan
              </h2>
            ) : usuario?.dias < cantDias && usuario?.tipoPlan !== "" ? (
              <h2 className="font-semibold mb-2 mx-auto text-center  w-full text-green-400 text-[15px]">
                ¡Estas al dia!
              </h2>
            ) : (
              <h2 className="font-semibold mb-2 mx-auto text-center  w-full text-gray-400 text-[15px]">
          ...
          <div className="p-5 py-2 text-left transform duration-500 hover:-translate-y-1 hover:shadow-xl cursor-pointer bg-black/50">
            <div className="flex  gap-x-4">
              <Image
                ...
                height={0}
              />
              <h2 className="font-semibold mb-2 mt-4 mx-auto w-full text-white/70 text-[20px]">
                Puntos obtenidos
              </h2>
            </div>
            ...
            <div className="progress mt-10 w-full">
              ...
              <div
                ref={divRef2}
                className="bg-green-500  h-2 rounded-md -mt-2"
              ></div>
            </div>
