...
export default function ControlIngresos() {
  ...
  const totals = filteredData.reduce((acc, item) => {
    ...
    acc[item.tipoPlan].amount += Number(
      item.monto
        .replace(/\./g, "")
        .replace(/\$/g, "")
        .replace(/[^0-9\.]/g, "")
  ...
  const totalMonto = Object.values(totals).reduce(
    (acc, { amount }) => acc + amount,
    0
  );
  const totalSuscripciones = Object.values(totals).reduce(
    (acc, { count }) => acc + count,
    0
  );
            ...
            <div className="text-center">
              ...
              <p className="text-3xl font-bold text-gray-800">
                ...
                {totalSuscripciones > 0
                  ? (totalMonto / totalSuscripciones).toFixed(2)
                  : "0.00"}
              </p>
            </div>
          </CardContent>
